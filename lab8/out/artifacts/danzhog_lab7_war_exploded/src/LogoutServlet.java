...
public class LogoutServlet extends ChatServlet
{
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        if (name!=null) {
            ...
            // Если идентификатор сессии пользователя, вошедшего под этим именем,
            // совпадает с идентификатором сессии пользователя, пытающегося выйти из чата (т.е. выходит тот же, кто и входил)
            if (aUser.getSessionId().equals((String) request.getSession().getId()))
            {
                // Удалить пользователя из списка активных
                // Т.к. запросы обрабатываются одновременно, нужна синхронизация
